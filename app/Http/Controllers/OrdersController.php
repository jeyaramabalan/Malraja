...
class OrdersController extends Controller
{
    ...
    {
        $this->datas['route'] = route("order.create");
        return view('orders.orders')->with($this->datas);
    }

    public function confirmedOrder(Request $request)
    {
        $this->datas['route'] = "get-order-confirmed";
        $this->datas['order_status'] = "1";
        return view('orders.orderssatus')->with($this->datas);
    }

    public function paymentPending(Request $request)
    {
        $this->datas['route'] = "get-order-payment";
        $this->datas['order_status'] = "3";
        return view('orders.orderssatus')->with($this->datas);
    }

    public function deliveryPending(Request $request)
    {
        $this->datas['route'] = "get-order-delivery";
        $this->datas['order_status'] = "2";
        return view('orders.orderssatus')->with($this->datas);
    }

    public function completedOrder(Request $request)
    {
        $this->datas['route'] = "get-order-completed";
        $this->datas['order_status'] = "4";
        return view('orders.orderssatus')->with($this->datas);
    }
    ...
    public function create(Request $request)
    {
        ...
        $this->datas['route'] = route("order.store");
        $this->datas['category'] = $category_option;
        return view('orders.add')->with($this->datas);
    }
    ...
    public function newPos(Request $request)
    {
        ...
        foreach($customers as $category) {
            ...
            //     $isFirst = false;
            // }
            $customers_option.= "<option $selected value={$category['id']}-{$category['name']}>{$category['mobile']}</option>";
        }

        $this->datas['customers'] = $customers_option;
        Session::forget('products');
        $this->datas['route'] = route("order-pos-store");
        $this->datas['category'] = $category_option;
        return view('orders.addpos')->with($this->datas);
    }
    ...
    public function posStore(Request $request)
    {
        ...
        //     $customerData = [];
        //     $customerData['mobile'] = $request->cust_id1;
        //     $customerData['created_by'] = Auth::id();
        //     if(!empty($request->name)) {
        //         $customerData['name'] = $request->name;
        //     } else {
        //         $customerData['name'] = "customer" . time();
        //     }
        ...
        OrderModel::where('id', $order['id'])->update(["bill_id" => sprintf('%08s', $order['id'])]);
        ...
        foreach(Session::get('products') as $cartArray) {
            ...
            OrderDetailModel::create($order_details);
        ...
        if(isset($request->isPrint) && $request->isPrint == "1") {
            if(isset($request->isGstPrint) && $request->isGstPrint == "1") {
                return redirect()->route('order-bill-gst', [$order['id']]);
            } else {
    ...
    public function update($id, Request $request)
    {
        try{
            ...
            $order_update_data = [];
            $order_update_data['customer_id'] = $request->cust_id;
            ...
            OrderModel::where('id', $id)->update($order_update_data);
            $orderDetailList = OrderDetailModel::where('order_id', $id)->get();
            ...
            if(count($orderDetailList) > 0) {
                foreach($orderDetailList as $detailsData) {
                    OrderDetailModel::where('id', $detailsData->id)->update(['status' => 0]);
            ...
            foreach(Session::get('products') as $cartArray) {
                $count = OrderDetailModel::where('order_id', $id)->where('product_id', $cartArray['id'])->count();
                $order_details = [];
                $order_details['quantity'] = $cartArray['count'];
                $order_details['amount'] = $cartArray['tgst'];
                $order_details['discount'] = 0;
                ...
                $stock['sale'] = $cartArray['count'];
                if($count > 0) {
                    OrderDetailModel::where('order_id', $id)->where('product_id', $cartArray['id'])->update($order_details);
                    StockModel::where('bill', $data['bill_id'])
            ...
            $output = array('success' => 1, 'msg' => 'Order Updated Successfully');
    ...
    {
        OrderModel::where('id', $request->id)->update(["status" => $request->status]);
    ...
    {
        OrderModel::where('status', $request->status)->update(["status" => $request->newStatus]);
        return "";
    }
    ...
    public function edit($id)
    {
        ...
        $this->datas['category'] = $category_option;
        Session::forget('products');
        return view('orders.edit')->with($this->datas);
    }

    public function getCustomerPendingPayment(Request $request)
    {
        $pendingAmount = OrderModel::where('customer_id', $request->id)
        ->where('status', '!=', 4)
    ...
    public function generateBill($id)
    {
        ...
        $order_items = OrderDetailModel::select(
    ...
    {
        $orderItems = OrderDetailModel::where('order_id', $request->id)->get();
    ...
    public function getOrderList(Request $request)
    {
        ...
        if(isset($request->status) && $request->status != "Select Status") {
            $visits = $visits->where('order.status', $request->status);
        } else {
            $visits = $visits->where('order.return_status', 0);
        }

        if(isset($request->types) && $request->types != "") {
            $visits = $visits->where('order.type', $request->types);
        }
        ...
        if(isset($request->search) && isset($request->search['value']) && $request->search['value'] != '') {
            $visits = $visits->where('order.id', 'like', '%'.$request->search['value'].'%');
        ...
        foreach($visits as $list)
        {
            ...
            if(Auth::user()->role == 1 || Auth::user()->role == 2 || Auth::user()->role == 3) {
                $edit  ='<a class="pl-2" href="'.route('order.edit', [$list->id]).'"><i style="color: green;" class="fa fa-edit"></i></a>';
            }
            $list->action       = '<td align="center">

                                        <a class="pl-2" href="'.route('order-bill', [$list->id]).'"><i style="color: orange;" class="fa fa-print"></i></a>
                                        <a class="pl-2" href="'.route('order-bill-gst', [$list->id]).'"><i style="color: blue;" class="fa fa-print"></i></a>
                                        <a class="pl-2" href="'.route('order.show', [$list->id]).'"><i style="color: primary;" class="fa fa-eye"></i></a>
                                        '.$edit.'
                                        '.$status.'
                                        '.$return.'
