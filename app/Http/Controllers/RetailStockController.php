...
class RetailStockController extends Controller
{
    public function getStock()
    {
        ...
        return view('retail_stock.stock')->with($this->datas);
    }

    public function getStocklist(Request $request)
    {
        $dataQry = RetailStockModel::select(
            DB::raw('SUM(retail_stock.sale) as sale'),
            DB::raw('SUM(retail_stock.purchase) as purchase'),
            DB::raw('SUM(retail_stock.sale_return) as sale_return'),
            DB::raw('SUM(retail_stock.purchase_return) as purchase_return'),
            DB::raw('SUM(retail_stock.purchase_free) as purchase_free'),
            DB::raw('SUM(retail_stock.sale_free) as sale_free'),
            DB::raw('SUM(retail_stock.sale_damage) as sale_damage'),
            DB::raw('SUM(retail_stock.purchase_damage) as purchase_damage'),
            DB::raw('SUM(retail_stock.purchase_damage_return) as purchase_damage_return'),
            DB::raw('SUM(retail_stock.sale_damage_return) as sale_damage_return'),
            'products.name as product_name',
            'products.status as deletedStatus',
        )
        ->leftjoin('products', 'retail_stock.product_id', '=', 'products.id');
        ...
        $dataQry = $dataQry->orderBy('products.name')->groupBy('retail_stock.product_id');
