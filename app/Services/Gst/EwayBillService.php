...
class EwayBillService
{
    public function preview(Request $request): array
    {
        ...
        $types = GstDateRange::parseDocTypes(
            $request->query('types'),
            array_keys(config('gst.doc_types'))
        );

        if ($scope === 'bill') {
            $docType = strtolower((string) $request->query('docType', ''));
            if (!in_array($docType, array_keys(config('gst.doc_types')), true)) {
            ...
            return [
                'scope' => $scope,
                'range' => null,
                'items' => [GstEligibility::mapPreviewRow($row, [GstEligibility::class, 'assessEway'])],
            ];
        }
        ...
        $items = array_map(function ($row) {
            return GstEligibility::mapPreviewRow($row, [GstEligibility::class, 'assessEway']);
        }, $rows);

        return ['scope' => $scope, 'range' => $range, 'items' => $items];
    }

    public function buildPdf(string $docType, int $id, array $transport): array
    {
        $doc = GstDocumentQuery::getDocument($docType, $id);
        if (!$doc) {
        ...
        $pdf = GstZipArchive::pdfFromView('reports.gst.eway_bill_pdf', [
            'doc' => $doc,
            'company' => $company,
            'transport' => $transport,
        ...
        return ['content' => $pdf, 'filename' => "eway-{$docType}-{$safe}.pdf"];
    }
    ...
    public function buildZip(Request $request): array
    {
        $scope = strtolower((string) $request->query('scope', 'day'));
        if ($scope === 'bill') {
            throw new \InvalidArgumentException('Use PDF endpoint for single bill');
        }
        ...
        $transport = GstEligibility::parseTransport($request->all());
        ...
        if ($selected !== []) {
            ...
            $targets = [];
            foreach ($rows as $row) {
                if (GstEligibility::assessEway($row)['eligible']) {
        ...
        foreach ($targets as $target) {
            $file = $this->buildPdf($target['doc_type'], $target['id'], $transport);
            $entries[] = ['name' => $file['filename'], 'content' => $file['content']];
        }
        ...
        return [
            ...
            'filename' => 'eway-bills_' . $label . '.zip',
        ];
    }
}
